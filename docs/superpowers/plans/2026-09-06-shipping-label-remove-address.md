...
**Architecture:** Keep the existing Dashboard-only HTML label generator. Remove address rendering and its CSS, strip whitespace from phone values, and release the summary block's old minimum height so products can use the reclaimed space.
...
Make `formatShippingPhone()` trim the value and remove whitespace. Remove address preparation, address markup, address CSS, and `min-height: 1.58in` from `.label-summary`.
