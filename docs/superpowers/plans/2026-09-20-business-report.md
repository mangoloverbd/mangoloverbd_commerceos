...
- Do not create or apply a migration. The live `orders` table already has `org_id`, `created_at`, `source`, `landing_page_path`, `status`, `fulfillment_status`, `price`, `delivery_rate`, `courier_fee`, `courier_status`, and `return_status`; its `(org_id, created_at desc)` index supports the report filter.
...
- Date bounds are an all-or-nothing inclusive calendar pair in `Asia/Dhaka`: `created_at >=` the local start midnight and `<` the next local midnight after the end date. No `23:59:59` sentinel. Bounded ranges may not include future Dhaka dates and are limited to 366 calendar days inclusive.
...
- Classify outcomes exclusively: an explicit courier cancellation/rejection is cancelled; a terminal return is returned/RTO even when legacy courier flows also mark the business status cancelled; remaining business or fulfillment cancellations are cancelled; then approved/progressing and pending. Shopify fulfillment state is a fallback for synced orders whose business status remains pending. Pending return requests are not RTO.
...
- Receives only the selected `orders` fields: `id`, `created_at`, `source`, `landing_page_path`, `status`, `fulfillment_status`, `price`, `delivery_rate`, `courier_fee`, `courier_status`, and `return_status`.
...
4. `classifyBusinessReportOutcome` normalizes `status`, `fulfillment_status`, `courier_status`, and `return_status` by lowercasing and replacing non-alphanumerics with underscores. Return `cancelled` for an explicit courier `cancelled`, `canceled`, or `rejected`; then return `returned` when business state is `returned`, return state is `returned`/`completed`, or courier state contains `return` **and does not** contain `request`, `pending`, `approval`, or `review`; then return `cancelled` for business or fulfillment cancellation; then return `approved` for an approved business state or fulfilled/partial Shopify fulfillment state; otherwise return `pending`. This preserves terminal courier returns from legacy flows that also set the business status to cancelled and prevents an in-progress return request from becoming an RTO outcome.
