            ...
            <div className="mt-3 shrink-0 border-t border-black/[0.08] bg-[#FAFAF8] pt-2">
              <div className="overflow-hidden rounded-lg bg-white ring-1 ring-inset ring-black/[0.06]">
                ...
                <div className="flex flex-wrap items-center gap-2 border-t border-black/[0.07] px-4 py-1.5">
                  ...
                  <div className="ml-auto flex items-center gap-1.5"><button type="button" onClick={() => navigate(returnTo)} disabled={creating} className="h-9 rounded-lg px-2.5 text-[12px] text-black hover:bg-black/[0.05] disabled:opacity-40">Cancel</button><button type="button" onClick={() => void createOrder()} disabled={creating} className="inline-flex h-9 items-center justify-center gap-1.5 rounded-lg bg-black px-3.5 text-[12px] font-medium text-white hover:bg-black/90 disabled:cursor-not-allowed disabled:opacity-40">{creating ? <Spinner size="sm" /> : <Plus weight="light" size={15} />}{creating ? "Creating…" : "Create order"}</button></div>
                </div>
              </div>
            </div>
          </section>
