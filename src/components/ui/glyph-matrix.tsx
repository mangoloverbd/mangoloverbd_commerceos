...
export function GlyphMatrix({
  ...
  // recolors the next frame without restarting the animation.
  ...
  useEffect(() => {
    ...
    // Only animate while the canvas is on screen (scrolled into view and not
    // display:none) — the loop stops entirely when it isn't.
    let visible = typeof IntersectionObserver === "undefined";

    const start = () => {
      if (reducedMotion || stopped || !visible || raf) return;
