          ...
          return (
            <article key={item.id} data-testid={`order-item-${item.id}`} className="rounded-lg bg-white p-4 ring-1 ring-inset ring-black/[0.06]">
              <div className="flex gap-3">
                ...
                <div className="min-w-0 flex-1 leading-snug"><h3 className="truncate text-[13px] font-medium text-black">{name}</h3>{item.product_slug && <p className="mt-0.5 truncate font-mono text-[10px] text-black/35">{item.product_slug}</p>}<p className="mt-1 text-[11px] text-black/40">{[item.variant_name, item.weight_kg ? `${item.weight_kg} kg` : null].filter(Boolean).join(" · ") || (isLegacy ? "Detached legacy item" : "Standard")}</p></div>
