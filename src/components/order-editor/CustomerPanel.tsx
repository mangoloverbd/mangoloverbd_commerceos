...
import { canEnterPrint, displayStatusLabel, isOnHoldStatus, isPrintStatus, normalizeBusinessStatus } from "@/lib/orderTransitions";
...
export function CustomerPanel({ order, customer, notes, disabled = false, history = [], historyLoading = false, onApply, onStatusChange, statusPending = false, onSaveNotes, notesPending = false }: CustomerPanelProps) {
        ...
        <div className="min-w-0">
          ...
          <div className="mt-2 flex items-center gap-2">
            <BuiSelect
              ...
              triggerClassName="h-9 capitalize text-[13px]"
            >
        ...
        {isOnHoldStatus(order.status) && (
          <div className="min-w-0 sm:col-span-2">
            ...
            <textarea
              ...
              className={`${inputClass} mt-2 h-auto min-h-24 py-2.5 normal-case tracking-normal`}
            />
            <div className="mt-2.5 flex items-center gap-2">
              <button
                ...
                className="inline-flex h-10 items-center gap-1.5 rounded-lg bg-black px-4 text-[13px] text-white disabled:opacity-40"
              >
                {notesPending ? <Spinner size="sm" /> : <Check weight="light" size={16} />} Save note
              </button>
            </div>
          </div>
        )}
