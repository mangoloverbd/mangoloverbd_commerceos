  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent align="start" className="w-72 rounded-lg border-black/[0.08] bg-[#FAFAF8] p-4 shadow-xl">
        ...
        <label className="mt-3 block text-[10px] font-medium uppercase tracking-[0.16em] text-black">Discount value<input aria-label="Discount value" type="number" min="0" max={mode === "percentage" ? 100 : item.unit_price} step="0.01" value={value} onChange={(event) => setValue(event.target.value)} className="mt-1.5 h-10 w-full rounded-lg bg-white px-3 font-mono text-[13px] outline-none ring-1 ring-inset ring-black/[0.08] focus:ring-black/25" /></label>
