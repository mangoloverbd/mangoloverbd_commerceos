  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <button type="button" disabled={disabled} aria-label={`${discountType ? "Edit" : "Add"} cart discount`} className="inline-flex h-7 items-center gap-1.5 rounded-lg px-2 text-[11px] text-black transition hover:bg-black/[0.05] disabled:opacity-30">
      ...
      <PopoverContent align="start" className="w-72 rounded-lg border-black/[0.08] bg-[#FAFAF8] p-4 shadow-xl">
        <div className="flex items-center justify-between"><p className="text-[11px] font-medium uppercase tracking-[0.16em] text-black">Cart discount</p><button type="button" aria-label="Close cart discount editor" onClick={() => setOpen(false)} className="grid h-7 w-7 place-items-center rounded-lg text-black hover:bg-black/[0.05]"><X weight="light" size={14} /></button></div>
        ...
        <label className="mt-3 block text-[10px] font-medium uppercase tracking-[0.16em] text-black">Discount value<input aria-label="Cart discount value" type="number" onWheel={blurOnWheel} min="0" max={mode === "percentage" ? 100 : base} step="0.01" value={value} onChange={(event) => setValue(event.target.value)} className="mt-1.5 h-10 w-full rounded-lg bg-white px-3 font-mono text-[13px] outline-none ring-1 ring-inset ring-black/[0.08] focus:ring-black/25" /></label>
        ...
        <div className="mt-3 rounded-lg bg-black/[0.04] p-3"><p className="text-[10px] uppercase tracking-[0.16em] text-black">Preview</p><p className="mt-1 font-mono text-[14px] tabular-nums text-black">−{formatTaka(previewAmount)} cart discount</p><p className="mt-0.5 text-[11px] text-black">{formatTaka(base - previewAmount)} merchandise total</p></div>
