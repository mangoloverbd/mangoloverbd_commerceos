...
test("both ingress paths verify context before limiting and hash signed device/network", () => {
  ...
  expect(limiter).toContain("checks.push([rlOrderDevice,");
  expect(limiter).toContain("checks.push([rlOrderNetwork,");
