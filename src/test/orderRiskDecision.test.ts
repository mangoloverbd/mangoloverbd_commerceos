...
describe("deterministic risk decision", () => {
  ...
  it("allowlist wins, but trust credit never cancels a critical signal", () => {
    ...
    const critical = decideRisk({ signals: signals("honeypot_filled", "trusted_delivered_customer"), contextTrusted: true });
    expect(critical).toMatchObject({ decision: "BLOCK", reasons: ["critical:honeypot_filled"] });
  });
  ...
  it("requires two high families to block at 100 points", () => {
    ...
    expect(independent).toMatchObject({ decision: "BLOCK", score: 100 });
  });

  it("holds location without delivered trust and holds uncertain infrastructure", () => {
    expect(decideRisk({ signals: signals("hater_region_ip"), contextTrusted: true }).decision).toBe("HOLD");
    ...
    for (const options of [{ contextTrusted: false }, { contextTrusted: true, dependencyUnavailable: true }, { contextTrusted: true, engineError: true }]) {
      expect(decideRisk({ signals: [], ...options }).decision).toBe("HOLD");
    }
