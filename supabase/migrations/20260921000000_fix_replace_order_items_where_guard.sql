...
-- The managed project rejects UPDATE statements without a WHERE clause
-- (even against session-local temp tables), so the blanket unit_discount
-- refresh below carries an always-true predicate. Every row in
-- order_item_edit has a product_id or a variant_id at this point (enforced
-- by the validation above), so no row is excluded.
