# AGENTS.md — Merchant-Suite Agent Guide
...
- **The storefront has a build-generated first-paint snapshot.** It is only a fast initial fallback. Supabase and the versioned public API remain authoritative, and the storefront revalidates live data in the background. A newly published product normally appears within about 8 seconds without a storefront deploy; a production storefront build refreshes the snapshot for future first loads.
